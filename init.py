...
def run(command):
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"Process - {command} stopped unexpectedly. Error code: ${result.returncode}")
...
def main():
    ...
    thread1 = threading.Thread(target=run, args=(["python", SMTPTranslator_path],))
    thread2 = threading.Thread(target=run, args=([Apache2_path],))
    thread3 = threading.Thread(target=run, args=([MySQL_path, "--defaults-file=../../mysql/bin/my.ini"],))
